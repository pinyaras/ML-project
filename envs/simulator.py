...
class NetworkSimulatorEnv(gym.Env):
    ...
    def __init__(self):
        ...
        self.total_hops = 0
        self.routing_time = []
        self.list_routed_packet = []
        self.current_event = event(0.0, 0) #do I need to do this?
        self.internode = 1.0 #initialize the transmission time to 1 for each step
        self.interqueue = 1.0
        ...
        # self.dests = [77, 21, 65, 35, 80, 59, 78, 72, 112, 2, 80, 101, 2, 57, 0,random. 13, 42, 72, 19, 56, 66, 22, 50, 80, 16, 24, 89, 110, 65, 113, 71, 87, 68, 52, 87, 112, 8, 95, 74, 79, 18, 50, 83, 101, 38, 31, 114, 109, 100, 58, 38, 53, 83, 11, 67, 57, 83, 49, 36, 80, 79, 60, 30, 102, 86, 69, 91, 37, 109, 28, 56, 65, 55, 42, 58, 4, 83, 16, 98, 96, 14, 66, 73, 97, 28, 40, 43, 28, 90, 40, 49, 74, 21, 59, 77, 34, 82, 103, 39, 23, 2, 36, 93, 68, 85, 88, 90, 115, 35, 27, 43, 103, 71, 3, 35, 6, 97, 7, 54, 10, 93, 35, 66]
    ...
    def step(self, action):
        ...
        if action < 0 or action not in self.links[current_node]:
            ...
            #next node = the select node from action
        ...
        if next_node == current_event.dest:
            ...
            self.temp =  current_time - current_event.birth + self.internode
            # self.temp = self.temp / self.routed_packets
            #self.routing_time.append(float(self.temp))
            #self.temp = 0
            ...
            # self.avg = self.total_routing_time/ self.routed_packets
            # self.avg_lists.append(self.avg)
    ...
    def pseudostep(self, action):
        ...
        if action < 0 or action not in self.links[current_node]:
            ...
            #next node = select action node
            next_node = self.links[current_node][action]
            #if next node is not dest
